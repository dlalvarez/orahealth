...
def test_evidence_json_has_minimum_structure(tmp_path):
    ...
    assert len(evidence["results"]) == 91
...
def test_execution_log_contains_run_metadata(tmp_path):
    ...
    assert "Loaded checks (91):" in log_text
    assert "Executed checks (87):" in log_text
    assert "Skipped checks (4):" in log_text
...
def test_oracle_features_are_rendered_in_technical_and_evidence_reports(tmp_path):
    output = _run_example(tmp_path)
    for report_name in ["technical_report.html", "evidence_report.html"]:
        html = (output / report_name).read_text(encoding="utf-8")
